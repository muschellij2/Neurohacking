...
setwd("~/Dropbox/Neurohacking/fslr_reg_kirby/")
@
...
library(oro.nifti)
library(kirby21)
t1_fname = get_image_filenames(ids = 113, visits = 1, modalities = "MPRAGE")
nim = readNIfTI(t1_fname, 
                reorient=FALSE)
@
\end{frame}
...
<<plot_fast_corr, eval=TRUE, echo = FALSE, cache=TRUE, results='hide'>>=
library(oro.nifti)
library(fslr)
library(scales)
png("Kirby21_Orig_Image.png")
orthographic(robust_window(nim), text="Original Image")
dev.off()
png("Kirby21_FAST_Image.png")
orthographic(robust_window(fast_img), text = "Bias-Corrected\n Image")
dev.off()
orig_wind = quantile(c(nim), probs = c(0, 0.999))
png("Kirby21_FAST_Image_Orig_Window.png")
...
png("Kirby21_FAST_Diff_Image.png")
orthographic(fast_diff, col = cols,
...
library(fslr)
library(scales)
png("Kirby21_N4_Image.png")
orthographic(robust_window(n4_img), text = "N4\nBias-Corrected\n Image")
dev.off()
orig_wind = quantile(c(nim), probs = c(0, 0.999))
png("Kirby21_N4_Image_Orig_Window.png")
orthographic(window_img(n4_img, window=orig_wind), 
             text = "Bias-Corrected\n Image")
dev.off()
n4_diff = nim - n4_img
png("Kirby21_N4_Diff_Image.png")
orthographic(n4_diff, col = cols,
             text = paste0("Original Image\nMinus N4", 
                                      "\nBias-Corrected\n Image"))
dev.off()
@
...
<<bet_neck, eval=TRUE, cache = TRUE>>=
bet_neck = fslbet(infile=nim, retimg=TRUE, 
                  betcmd = "bet",
                  opts = "-B -v" )
@
\end{frame}
...
dev.off()
png("BET_Image3.png")
orthographic(robust_window(bet_neck), 
             text="Post-Neck Removal\n Brain-Extracted\n Image")
dev.off()
...
registered_fast_brain = flirt(infile=bet_neck, 
...
affine_fast_brain = flirt(infile=bet_neck, 
                          reffile = "MNI152_T1_1mm_brain.nii.gz",
                          dof = 12,
                          retimg = TRUE)
...
fnirt_fast = fnirt_with_affine(infile=bet_neck, 
                               reffile = "MNI152_T1_1mm_brain.nii.gz",
